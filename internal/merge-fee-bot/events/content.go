...
func getPaymentParams(pr *github.PullRequest) *PaymentsParams {
	...
	for i := range payments {
		...
		payments[i].MinAmount = getMinAmount(
			config.Default.Payment.AmountTolerancePercent, config.Default.Payment.Assets[i].Amount)
		payments[i].Memo = memo
		payments[i].CreatedTime = createdTime
		payments[i].EndTime = createdTime + monthInSec
	}

	qrTw, qrFull := getQR(config.Default.Payment.Assets[0].Amount, config.Default.Payment.Address, memo)
