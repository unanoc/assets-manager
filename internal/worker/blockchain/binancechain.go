...
func NewClient() *Client {
	...
	c, err := client.NewDexClient(config.Default.ClientURLs.BinanceDEX, types.ProdNetwork, keyManager)
	if err != nil {
	...
	return &Client{
		client:       c,
		customClient: api.InitClient(config.Default.ClientURLs.BinanceAPI, nil),
	}
}
...
func (c *Client) BurnToken(token string, amount int64) (string, error) {
	...
	return fmt.Sprintf("%s/tx/%s", config.Default.ClientURLs.BinanceExplorer, res.Hash), nil
}
